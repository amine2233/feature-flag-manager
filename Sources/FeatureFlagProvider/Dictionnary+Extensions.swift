...
public enum BentoDict {
    ...
    internal static func removeValue(_ dict: inout [String: Any], for keyPath: FeatureFlagKeyPath) {
        switch keyPath.count {
            ...
            case (2..<Int.max):
                ...
                removeValue(&subDict, for: keyPath.dropFirst())
                dict[key] = subDict
            default:
                return
        }
    ...
    ///   - keys: keypath.
    /// - Returns: typed value.
    internal static func getValueInDictionary<V>(_ dict: [String: Any], for keys: FeatureFlagKeyPath) -> V? {
        switch keys.count {
            case 1:
